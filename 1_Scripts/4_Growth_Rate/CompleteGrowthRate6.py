...
def G_Rate_function(M1, M2):
    #0th step: add a row in each matrix from the other matrix
    # this seems erratic but we dont want to have vector-matrices
    # so this helps out
    M1_New = M1[0]
    M2_New = M2[0]
    M1 = np.vstack((M1, M2_New))
    M2 = np.vstack((M2, M1_New))
